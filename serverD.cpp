...
int main(){
    unordered_map<string, int> Double_rooms;
    Double_rooms = readRoomInfo("double.txt");
    ...
    cout<<"Server D is up and running using UDP on port "<<PORT_NUMBER_D<<" ."<<endl;
    // send info to Main
    int state_index;
    for (const auto& pair : Double_rooms) {
        int state_index;
        string update_text;
    ...
    while(true){
        ...
        if (action=="Availability"){
            ...
            // from Beej
            if (Double_rooms.find(roomcode) != Double_rooms.end()){
                if (Double_rooms[roomcode] > 0){
                    response = 0;
            ...
            cout<<"The Server D received an reservation request from the main server."<<endl;
            if (Double_rooms.find(roomcode) != Double_rooms.end()){
                if (Double_rooms[roomcode] > 0){
                    response = 0;
                    Double_rooms[roomcode] -= 1;
                    cout<<"Successful reservation. The count of Room "<<roomcode<<" is now "<<Double_rooms[roomcode]<<" ."<<endl;
                    cout << "The Server D finished sending the response and the updated room status to the main server. "<<endl; //还需要完善一下
                }else{
                    response = 1;
