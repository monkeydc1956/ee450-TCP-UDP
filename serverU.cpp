...
int main(){
    unordered_map<string, int> Suite_rooms;
    Suite_rooms = readRoomInfo("suite.txt");
    ...
    cout<<"Server U is up and running using UDP on port "<<PORT_NUMBER_D<<" ."<<endl;
    // send info to Main
    int state_index;
    for (const auto& pair : Suite_rooms) {
        int state_index;
        string update_text;
    ...
    while(true){
        ...
        if (action=="Availability"){
            ...
            if (Suite_rooms.find(roomcode) != Suite_rooms.end()){
                if (Suite_rooms[roomcode] > 0){
                    response = 0;
            ...
            cout<<"The Server U received an reservation request from the main server."<<endl;
            if (Suite_rooms.find(roomcode) != Suite_rooms.end()){
                if (Suite_rooms[roomcode] > 0){
                    response = 0;
                    Suite_rooms[roomcode] -= 1;
                    cout<<"Successful reservation. The count of Room "<<roomcode<<" is now "<<Suite_rooms[roomcode]<<" ."<<endl;
                    cout << "The Server U finished sending the response and the updated room status to the main server. "<<endl; //还需要完善一下
                }else{
                    response = 1;
